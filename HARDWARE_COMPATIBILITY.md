...
| `KE4` | against the development peer: pull and reinsert the cable twenty times and restart each side independently, with no repair step; the USB identity the Pico presents recorded | STEP ONE OBSERVED, NOT CLEARED. Step one, on the Windows PC with `scripts/link_check.ps1 -Port COM8`, reported by the author 2026-09-15: HELLO on open, retried at two seconds, silenced by the DISPLAY acceptance; `CENTER_SHORT`, `CENTER_LONG` and `RIGHT_SHORT` each arrived with both guard flags true; the port closed cleanly. Step two, on the Pi (Linux): `make peer`, run `build/host/development_peer /dev/ttyACM0` (or the node `dmesg` names), drive states from its prompt, then twenty cable pulls and a restart of each side; record `lsusb -v -d 2e8a:0009` and `udevadm info -a` for the tty, twice across a reboot, into the evaluation log. |
